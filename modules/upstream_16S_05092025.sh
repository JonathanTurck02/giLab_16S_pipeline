...
trunclenF=200 # 200 seems to be a pretty good standard cutoff (however, this may change based on qc)
trunclenR=200  
user="your_netID"
mappingFile="metadata.txt" # qiime2 metadata file
# remember to copy script_bank into your scratch directory

# STEP1: DADA2 ASV identification
qiime dada2 denoise-paired --i-demultiplexed-seqs trimmed-demux.qza --p-trunc-len-f ${trunclenF} --p-n-threads 48 --p-trunc-len-r ${trunclenR} --o-representative-sequences asv-seqs.qza --o-table feature-table.qza --o-denoising-stats denoising-stats.qza
