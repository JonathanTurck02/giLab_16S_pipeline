...
# ================================================== USER INPUT ================== # Insert sampling depth, mapping file, and column of interest
samplingDepth=200000  
mappingFile="mapping_file.txt"
columnName='Treatment Group'
...
qiime taxa collapse --i-table rarefied-filtered-feature-table-${samplingDepth}.qza --i-taxonomy taxonomy.qza --p-level 8 --o-collapsed-table taxa-by-level/collapsed-filtered-table-L8.qza

qiime tools export --input-path taxa-by-level/collapsed-filtered-table-L8.qza --output-path taxa-by-level/L8-strain

biom convert -i taxa-by-level/L8-strain/feature-table.biom -o taxa-by-level/L8-strain/L8-strain-table.tsv --to-tsv

# STEP7: Generate core-metrics, alpha and beta diversity analyses
qiime diversity core-metrics --i-table rarefied-filtered-feature-table-${samplingDepth}.qza --p-sampling-depth ${samplingDepth} --m-metadata-file ${mappingFile} --output-dir core-metrics-outputs
...
qiime diversity alpha-rarefaction --i-table filtered-feature-table.qza --p-max-depth ${samplingDepth} --p-metrics chao1 --m-metadata-file ${mappingFile} --o-visualization alpha_rarefaction_plots/chao1-plot.qzv

qiime diversity alpha-rarefaction --i-table filtered-feature-table.qza --p-max-depth ${samplingDepth} --p-metrics shannon --m-metadata-file ${mappingFile} --o-visualization alpha_rarefaction_plots/shannon-plot.qzv

qiime diversity alpha-rarefaction --i-table filtered-feature-table.qza --p-max-depth ${samplingDepth} --p-metrics goods_coverage --m-metadata-file ${mappingFile} --o-visualization alpha_rarefaction_plots/goods_coverage-plot.qzv
...
qiime composition add-pseudocount --i-table taxa-by-level/collapsed-filtered-table-L8.qza --o-composition-table ANCOM-QZA/composition-table-L8.qza
  
qiime composition ancom --i-table ANCOM-QZA/composition-table-L8.qza --m-metadata-file ${mappingFile} --m-metadata-column ${columnName} --o-visualization ANCOM/ancom-${columnName}-L8.qzv
...
qiime composition tabulate --i-data L8_differential_abundance_by_${columnName}.qza --o-visualization ANCOMBC/L8_differential_abundance_by_${columnName}.qzv

qiime composition da-barplot --i-data L8_differential_abundance_by_${columnName}.qza --o-visualization DA_barplot_L8.qzv

qiime tools export --input-path L8_differential_abundance_by_${columnName}.qza --output-path ANCOMBC/exports/L8-export
